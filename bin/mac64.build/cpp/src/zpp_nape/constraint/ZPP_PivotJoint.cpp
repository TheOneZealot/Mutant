...
Void ZPP_PivotJoint_obj::validate( ){
{
		...
		if ((tmp18)){
			...
			HX_STACK_LINE(485)
			::String tmp20 = tmp19->outer->toString();		HX_STACK_VAR(tmp20,"tmp20");
			HX_STACK_LINE(485)
			::String tmp21 = (HX_HCSTRING("Error: Constraints cannot have both bodies non-dynamic (body1=","\xe1","\xda","\xd7","\x49") + tmp20);		HX_STACK_VAR(tmp21,"tmp21");
			HX_STACK_LINE(485)
			::String tmp22 = (tmp21 + HX_HCSTRING(", body2=","\x61","\x88","\x43","\x62"));		HX_STACK_VAR(tmp22,"tmp22");
			HX_STACK_LINE(485)
			::zpp_nape::phys::ZPP_Body tmp23 = this->b2;		HX_STACK_VAR(tmp23,"tmp23");
			HX_STACK_LINE(485)
			::String tmp24 = tmp23->outer->toString();		HX_STACK_VAR(tmp24,"tmp24");
			HX_STACK_LINE(485)
