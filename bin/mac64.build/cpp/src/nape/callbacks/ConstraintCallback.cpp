...
::String ConstraintCallback_obj::toString( ){
	...
	HX_STACK_LINE(200)
	::String tmp3 = Array_obj< ::String >::__new().Add(HX_HCSTRING("WAKE","\x24","\x24","\xb3","\x39")).Add(HX_HCSTRING("SLEEP","\xf7","\xf4","\xb2","\xfc")).Add(HX_HCSTRING("BREAK","\x9f","\x5c","\xda","\x36"))->__get(tmp2);		HX_STACK_VAR(tmp3,"tmp3");
	HX_STACK_LINE(200)
	hx::AddEq(ret,tmp3);
	HX_STACK_LINE(201)
	::zpp_nape::callbacks::ZPP_Callback tmp4 = this->zpp_inner;		HX_STACK_VAR(tmp4,"tmp4");
	HX_STACK_LINE(201)
	::nape::constraint::Constraint tmp5 = tmp4->constraint->outer;		HX_STACK_VAR(tmp5,"tmp5");
	HX_STACK_LINE(201)
